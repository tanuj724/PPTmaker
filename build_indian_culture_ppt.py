...
Topic: The Living Civilization: Five Millennia of Indian Culture, Philosophy, and Heritage
Visual Theme: Sandalwood & Peacock Heritage
"""
import os
import sys
import json
...
# ---------------------------------------------------------------------------
# COLOR PALETTE (Sandalwood & Peacock Heritage)
# ---------------------------------------------------------------------------
PRIMARY_DARK      = RGBColor(0x1A, 0x28, 0x36)  # Deep Royal Indigo / Peacock Midnight
SECONDARY_DARK    = RGBColor(0x2E, 0x1D, 0x16)  # Deep Sandalwood Umber
ACCENT_METAL      = RGBColor(0xD4, 0x9B, 0x24)  # Royal Temple Gold
ACCENT_METAL_LIGHT= RGBColor(0xF7, 0xDC, 0x8B)  # Sunlit Warm Gold
ACCENT_SAFFRON    = RGBColor(0xE6, 0x51, 0x00)  # Sacred Saffron / Kesari
ACCENT_MARIGOLD   = RGBColor(0xF5, 0x9E, 0x0B)  # Marigold Ochre
ACCENT_PEACOCK    = RGBColor(0x0C, 0x62, 0x7A)  # Peacock Blue
CANVAS            = RGBColor(0xFA, 0xF6, 0xEE)  # Sandstone Parchment
SURFACE_ALT       = RGBColor(0xF2, 0xEA, 0xDC)  # Warm Linen Surface
SURFACE_WHITE     = RGBColor(0xFF, 0xFF, 0xFF)  # Pure White
TEXT_INK          = RGBColor(0x1E, 0x1E, 0x1C)  # Deep Charcoal Ink
TEXT_MUTED        = RGBColor(0x67, 0x64, 0x5E)  # Warm Muted Slate
RULE_HAIRLINE     = RGBColor(0xE0, 0xD7, 0xC5)  # Fine Sandstone Rule
ON_DARK_TEXT      = RGBColor(0xF2, 0xEB, 0xE0)  # Soft Parchment on Dark
...
ASSETS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets_indian_culture")
SPEC_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "research_json", "content_spec_indian_culture.json")
...
def add_bullet_list(slide, l, t, w, h, items, size=15.5, spacing=1.10, space_after=10):
    tb = slide.shapes.add_textbox(Inches(l), Inches(t), Inches(w), Inches(h))
    tf = tb.text_frame
    tf.word_wrap = True
...
def add_footer(slide, slide_num):
    ...
    add_text_box(slide, 0.80, 7.08, 8.50, 0.28,
                 "The Living Civilization: Five Millennia of Indian Culture, Philosophy, and Heritage",
...
def add_stat_card_widget(slide, l, t, w, h, headline, sub=None, bg_color=PRIMARY_DARK, accent_color=ACCENT_METAL):
    ...
    if sub:
        add_rect(slide, l + 0.28, t + 0.82, w - 0.56, 0.012, ACCENT_METAL)
...
def render_slide_1_hero_cover(prs, spec):
    ...
    if img:
        ...
        # Re-add subtle translucent effect via foreground card
    ...
    # Kicker Badge
    badge_w, badge_h = 2.4, 0.36
    badge_l = card_l + 0.60
    badge_t = card_t + 0.55
    add_rect(slide, badge_l, badge_t, badge_w, badge_h, ACCENT_SAFFRON, shape_type=MSO_SHAPE.ROUNDED_RECTANGLE, adj=0.2)
    add_text_box(slide, badge_l, badge_t + 0.04, badge_w, badge_h, spec.get("kicker", "CULTURAL MASTERCLASS").upper(),
                 size=10, bold=True, color=WHITE, align=PP_ALIGN.CENTER)

    # Hero Title
    add_text_box(slide, card_l + 0.60, card_t + 1.15, card_w - 1.20, 1.25,
                 spec.get("title", "The Living Civilization"),
                 size=52, bold=True, color=WHITE, font=TITLE_FONT, spacing=1.05)

    # Subtitle
    add_text_box(slide, card_l + 0.60, card_t + 2.50, card_w - 1.20, 0.90,
                 spec.get("subtitle", "Five Millennia of Indian Culture, Philosophy, and Global Resonance"),
                 size=20, bold=False, color=ACCENT_METAL_LIGHT, font=BODY_FONT, spacing=1.15)

    # Golden Divider
    add_rect(slide, card_l + 0.60, card_t + 3.65, card_w - 1.20, 0.015, ACCENT_METAL)

    # Meta Info Row
    meta_runs = [
        {"text": "AUTHOR & CURATION:  ", "bold": True, "size": 10, "color": ACCENT_METAL_LIGHT},
        {"text": "PPTmaker Cultural Series      ", "bold": False, "size": 10, "color": ON_DARK_TEXT},
        {"text": "ARCHETYPE:  ", "bold": True, "size": 10, "color": ACCENT_METAL_LIGHT},
        {"text": "Heroic Legacy Arc      ", "bold": False, "size": 10, "color": ON_DARK_TEXT},
        {"text": "TIMELINE:  ", "bold": True, "size": 10, "color": ACCENT_METAL_LIGHT},
        {"text": "5,000 BCE — Present", "bold": False, "size": 10, "color": ON_DARK_TEXT},
    ]
...
def render_slide_2_dual_stat(prs, spec):
    ...
    add_header(slide, spec.get("kicker", "SCALE & ANTIQUITY"), spec.get("title", "A Civilization Measured in Millennia"))
    ...
    # Huge numeral
    add_text_box(slide, col1_l + 0.35, card_a_t + 0.18, 4.20, 0.90, stat_a.get("numeral", "5,000+"),
                 size=54, bold=True, color=ACCENT_METAL_LIGHT, font=TITLE_FONT)
    # Headline
    add_text_box(slide, col1_l + 0.35, card_a_t + 1.10, 4.20, 0.42, stat_a.get("headline", "Years of Living Continuity"),
    ...
    # Stat Card B (1.4B+ Custodians)
    card_b_t = 4.35
    add_rect(slide, col1_l, card_b_t, 4.80, card_h, SECONDARY_DARK, shape_type=MSO_SHAPE.ROUNDED_RECTANGLE, adj=0.04)
    add_rect(slide, col1_l, card_b_t, 0.12, card_h, ACCENT_SAFFRON)
    # Huge numeral
    add_text_box(slide, col1_l + 0.35, card_b_t + 0.18, 4.20, 0.90, stat_b.get("numeral", "1.4B+"),
                 size=54, bold=True, color=ACCENT_METAL_LIGHT, font=TITLE_FONT)
    # Headline
    add_text_box(slide, col1_l + 0.35, card_b_t + 1.10, 4.20, 0.42, stat_b.get("headline", "Living Heritage Custodians"),
                 size=14.5, bold=True, color=WHITE, font=BODY_FONT)
    # Description
    ...
    add_text_box(slide, col2_l + 0.40, 1.95, 5.80, 0.35, "CIVILIZATIONAL CONTINUITY",
                 size=11, bold=True, color=ACCENT_SAFFRON, font=BODY_FONT)
    add_text_box(slide, col2_l + 0.40, 2.25, 5.80, 0.55, "A Living Heritage Unmatched in Human History",
...
def render_slide_3_timeline(prs, spec):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_bg_color(slide, CANVAS)
    add_header(slide, spec.get("kicker", "CIVILIZATIONAL EPOCHS"), spec.get("title", "From Harappan Urbanism to Devotional Renaissance"))
    ...
    # Connecting horizontal ribbon behind cards
    add_rect(slide, start_l, top_t + 0.50, 11.733, 0.08, ACCENT_METAL)
    ...
    for i, m in enumerate(milestones):
        ...
        add_text_box(slide, cl + 0.10, top_t + 0.10, col_w - 0.20, 0.30, m.get("period", ""),
                     size=11, bold=True, color=ACCENT_METAL_LIGHT, align=PP_ALIGN.CENTER, font=BODY_FONT)

        # Milestone step circle marker
        step_sz = 0.36
        step_l = cl + (col_w - step_sz) / 2.0
        step_t = top_t + 0.62
        add_rect(slide, step_l, step_t, step_sz, step_sz, ACCENT_METAL, line_color=WHITE, line_w=1.5,
    ...
    add_stat_card_widget(slide, 0.80, 5.55, 11.733, 1.25,
                        stat.get("headline", "5,000-Year Arc"),
                        stat.get("sub", "An unbroken lineage of architectural, philosophical, and literary innovation"))
...
def render_slide_4_three_pillars(prs, spec):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_bg_color(slide, CANVAS)
    add_header(slide, spec.get("kicker", "PHILOSOPHICAL BEDROCK"), spec.get("title", "Three Universal Pillars That Anchor Indian Thought"))
    ...
    theme_accents = [ACCENT_SAFFRON, ACCENT_METAL, ACCENT_PEACOCK]
    ...
    for i, p in enumerate(pillars):
        ...
        add_text_box(slide, cl + 0.30, top_t + 0.40, badge_w, 0.28, p.get("badge", "Pillar").upper(),
        ...
        add_text_box(slide, cl + 0.30, top_t + 0.82, card_w - 0.60, 0.65, p.get("title", ""),
                     size=17, bold=True, color=PRIMARY_DARK, font=TITLE_FONT, spacing=1.05)
...
def render_slide_5_architecture(prs, spec):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_bg_color(slide, CANVAS)
    add_header(slide, spec.get("kicker", "SACRED ARCHITECTURE"), spec.get("title", "Stone Carved into Cosmic Geometry and Devotion"))
    ...
    add_stat_card_widget(slide, col1_l, 5.55, 6.80, 1.25,
                        stat.get("headline", "42 UNESCO Sites"),
                        stat.get("sub", "34 cultural world heritage monuments protected across the Indian subcontinent"))

    # Right Column: Architectural Photo Card (Width = 4.60")
    col2_l = 7.933
    img_h = 5.10
    img = asset_path("architecture")
    if img:
        # Photo container
        ...
        # Overlay Caption Bar at bottom of photo
        cap_h = 0.80
        cap_t = 1.70 + img_h - cap_h
        add_rect(slide, col2_l, cap_t, 4.60, cap_h, PRIMARY_DARK)
        add_rect(slide, col2_l, cap_t, 4.60, 0.04, ACCENT_METAL)
        add_text_box(slide, col2_l + 0.25, cap_t + 0.12, 4.10, 0.28, "SACRED GEOMETRY IN STONE",
                     size=9.5, bold=True, color=ACCENT_METAL_LIGHT, font=BODY_FONT)
        add_text_box(slide, col2_l + 0.25, cap_t + 0.38, 4.10, 0.35, "Sun Temple Konark & Kailasa Monolith",
...
def render_slide_6_stat_grid(prs, spec):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_bg_color(slide, CANVAS)
    add_header(slide, spec.get("kicker", "DEMOGRAPHIC MOSAIC"), spec.get("title", "The World's Most Pluralistic Society by the Numbers"))
    ...
    accents = [ACCENT_METAL, ACCENT_SAFFRON, ACCENT_PEACOCK, ACCENT_MARIGOLD]
    ...
    for i, c in enumerate(cards):
        ...
        add_text_box(slide, cl + 0.40, ct + 0.20, card_w - 0.60, 0.85, c.get("numeral", ""),
                     size=50, bold=True, color=ACCENT_METAL_LIGHT, font=TITLE_FONT)
        # Headline
...
def render_slide_7_quote(prs, spec):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_bg_color(slide, CANVAS)
    add_header(slide, spec.get("kicker", "CIVILIZATIONAL HARMONY"), spec.get("title", "A Universal Message of Tolerance and Acceptance"))

    # Left Column: Swami Vivekananda Portrait Card (Width = 3.60")
    col1_l = 0.80
    col1_w = 3.60
    img_h = 5.10
    ...
    if img:
        ...
        add_text_box(slide, col1_l + 0.20, 1.70 + img_h - 0.65, col1_w - 0.40, 0.25, "SWAMI VIVEKANANDA",
                     size=10, bold=True, color=ACCENT_METAL_LIGHT, font=BODY_FONT, align=PP_ALIGN.CENTER)
        add_text_box(slide, col1_l + 0.20, 1.70 + img_h - 0.40, col1_w - 0.40, 0.30, "Chicago, 1893",
    ...
    add_rect(slide, col2_l, 1.70, col2_w, 5.10, SURFACE_WHITE, line_color=RULE_HAIRLINE, line_w=1.0,
             shape_type=MSO_SHAPE.ROUNDED_RECTANGLE, adj=0.03)
    add_rect(slide, col2_l, 1.70, col2_w, 0.12, ACCENT_SAFFRON)

    # Decorative Quote Mark
    add_text_box(slide, col2_l + 0.50, 1.95, 1.20, 0.80, "\u201C",
                 size=60, bold=True, color=ACCENT_METAL, font=TITLE_FONT)

    # Quote Body
    quote_text = f"\"{spec.get('quote', '')}\""
    add_text_box(slide, col2_l + 0.50, 2.55, col2_w - 1.00, 1.65, quote_text,
                 size=17.5, bold=False, italic=True, color=PRIMARY_DARK, font=TITLE_FONT, spacing=1.18)

    # Speaker & Context
    add_rect(slide, col2_l + 0.50, 4.30, col2_w - 1.00, 0.012, RULE_HAIRLINE)
    add_text_box(slide, col2_l + 0.50, 4.45, col2_w - 1.00, 0.35, spec.get("speaker", "Swami Vivekananda"),
                 size=14, bold=True, color=ACCENT_SAFFRON, font=BODY_FONT)
    add_text_box(slide, col2_l + 0.50, 4.75, col2_w - 1.00, 0.30, spec.get("context", ""),
                 size=10.5, bold=False, color=TEXT_MUTED, font=BODY_FONT)

    # Supporting analytical commentary
    supp = spec.get("supporting_text", "")
    if supp:
        add_text_box(slide, col2_l + 0.50, 5.15, col2_w - 1.00, 1.40, supp,
...
def render_slide_8_table(prs, spec):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_bg_color(slide, CANVAS)
    add_header(slide, spec.get("kicker", "PERFORMING ARTS"), spec.get("title", "The Six Great Dimensions of Classical Indian Dance"))

    table_data = spec.get("table", {})
    cols = table_data.get("columns", ["Dance Form", "Origin & Region", "Treatise", "Signature Aesthetics"])
    ...
    col_widths = [Inches(2.20), Inches(2.00), Inches(2.60), Inches(4.933)]
    ...
    for c_idx, col_name in enumerate(cols):
        ...
        run.font.size = Pt(10)
        run.font.bold = True
        run.font.color.rgb = ACCENT_METAL_LIGHT

    # Data Rows
    for r_idx, row in enumerate(rows):
        row_bg = SURFACE_ALT if (r_idx % 2 == 1) else SURFACE_WHITE
        row_cells = [row.get("name", ""), row.get("region", ""), row.get("treatise", ""), row.get("focus", "")]
...
def render_slide_9_festivals(prs, spec):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_bg_color(slide, CANVAS)
    add_header(slide, spec.get("kicker", "LIVING CELEBRATIONS"), spec.get("title", "A Calendar Governed by Seasons, Moons, and Sacred Light"))

    # Inverted Asymmetric Layout: Image on LEFT (Width = 4.60"), Bullets on RIGHT (Width = 6.80")
    col1_l = 0.80
    img_h = 5.10
    img = asset_path("festivals")
    if img:
        ...
        add_rect(slide, col1_l, cap_t, 4.60, 0.04, ACCENT_SAFFRON)
        add_text_box(slide, col1_l + 0.25, cap_t + 0.12, 4.10, 0.28, "FESTIVAL OF LIGHTS & RENEWAL",
                     size=9.5, bold=True, color=ACCENT_METAL_LIGHT, font=BODY_FONT)
        add_text_box(slide, col1_l + 0.25, cap_t + 0.38, 4.10, 0.35, "Diwali Diyas & Sacred Traditions",
                     size=11, bold=False, color=WHITE, font=TITLE_FONT)
    ...
    add_stat_card_widget(slide, col2_l, 5.55, 6.80, 1.25,
                        stat.get("headline", "UNESCO Intangible"),
                        stat.get("sub", "Kumbh Mela, Durga Puja, and Vedic chanting inscribed on world heritage lists"),
                        bg_color=SECONDARY_DARK, accent_color=ACCENT_SAFFRON)
...
def render_slide_10_mosaic(prs, spec):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    set_bg_color(slide, CANVAS)
    add_header(slide, spec.get("kicker", "GLOBAL IMPACT"), spec.get("title", "Ancient Wisdom Reimagined as 21st-Century Soft Power"))
    ...
    # Top Row: 2 Split Cards (Width = 5.68" each)
    top_w = 5.68
    top_h = 2.55
    top_t = 1.70
    gap_x = 0.373

    # Top Left Card (Yoga & Wellness)
    c1_l = 0.80
    add_rect(slide, c1_l, top_t, top_w, top_h, SURFACE_WHITE, line_color=RULE_HAIRLINE, line_w=1.0,
             shape_type=MSO_SHAPE.ROUNDED_RECTANGLE, adj=0.03)
    add_rect(slide, c1_l, top_t, top_w, 0.10, ACCENT_PEACOCK)
    # Badge
    add_rect(slide, c1_l + 0.30, top_t + 0.25, 1.70, 0.30, SURFACE_ALT, line_color=ACCENT_PEACOCK, line_w=0.75,
             shape_type=MSO_SHAPE.ROUNDED_RECTANGLE, adj=0.2)
    add_text_box(slide, c1_l + 0.30, top_t + 0.29, 1.70, 0.25, card1.get("badge", "Wellness").upper(),
                 size=8.5, bold=True, color=ACCENT_PEACOCK, align=PP_ALIGN.CENTER)
    ...
    # Top Right Card (Diaspora)
    c2_l = 0.80 + top_w + gap_x
    add_rect(slide, c2_l, top_t, top_w, top_h, SURFACE_WHITE, line_color=RULE_HAIRLINE, line_w=1.0,
             shape_type=MSO_SHAPE.ROUNDED_RECTANGLE, adj=0.03)
    add_rect(slide, c2_l, top_t, top_w, 0.10, ACCENT_SAFFRON)
    # Badge
    add_rect(slide, c2_l + 0.30, top_t + 0.25, 1.85, 0.30, SURFACE_ALT, line_color=ACCENT_SAFFRON, line_w=0.75,
             shape_type=MSO_SHAPE.ROUNDED_RECTANGLE, adj=0.2)
    add_text_box(slide, c2_l + 0.30, top_t + 0.29, 1.85, 0.25, card2.get("badge", "Diaspora").upper(),
                 size=8.5, bold=True, color=ACCENT_SAFFRON, align=PP_ALIGN.CENTER)
    ...
    # Bottom Full-Width Synthesis Banner (Width = 11.733", Height = 2.25")
    bot_l = 0.80
    bot_t = 4.45
    bot_w = 11.733
    bot_h = 2.35
    ...
    add_rect(slide, bot_l + 0.40, bot_t + 0.25, 1.90, 0.32, SECONDARY_DARK, line_color=ACCENT_METAL, line_w=0.75,
             shape_type=MSO_SHAPE.ROUNDED_RECTANGLE, adj=0.2)
    add_text_box(slide, bot_l + 0.40, bot_t + 0.30, 1.90, 0.25, card3.get("badge", "Synthesis").upper(),
                 size=8.5, bold=True, color=ACCENT_METAL_LIGHT, align=PP_ALIGN.CENTER)
    # Title
    add_text_box(slide, bot_l + 0.40, bot_t + 0.68, bot_w - 0.80, 0.42, card3.get("title", "Continuity in Modernity"),
                 size=16, bold=True, color=WHITE, font=TITLE_FONT)
    # Subtitle
    add_text_box(slide, bot_l + 0.40, bot_t + 1.10, bot_w - 0.80, 0.30, card3.get("subtitle", ""),
                 size=12, bold=True, color=ACCENT_METAL_LIGHT, font=BODY_FONT)
    # Body
...
def render_slide_11_hero_closing(prs, spec):
    ...
    badge_t = card_t + 0.55
    add_rect(slide, badge_l, badge_t, badge_w, badge_h, ACCENT_SAFFRON, shape_type=MSO_SHAPE.ROUNDED_RECTANGLE, adj=0.2)
    add_text_box(slide, badge_l, badge_t + 0.04, badge_w, badge_h, spec.get("kicker", "THE TIMELESS LEGACY").upper(),
                 size=10, bold=True, color=WHITE, align=PP_ALIGN.CENTER)

    # Hero Title
    add_text_box(slide, card_l + 0.60, card_t + 1.15, card_w - 1.20, 1.15,
                 spec.get("title", "Continuity in Transformation"),
                 size=48, bold=True, color=WHITE, font=TITLE_FONT, spacing=1.05)

    # Subtitle
    add_text_box(slide, card_l + 0.60, card_t + 2.40, card_w - 1.20, 0.85,
                 spec.get("subtitle", "The Eternal Flow of Bharat — Rooted in Five Millennia, Inspiring the Global Future"),
                 size=18.5, bold=False, color=ACCENT_METAL_LIGHT, font=BODY_FONT, spacing=1.15)

    # Stat callout box at bottom of card
    stat = spec.get("stat_callout", {})
    add_rect(slide, card_l + 0.60, card_t + 3.45, card_w - 1.20, 1.40, SECONDARY_DARK, line_color=ACCENT_METAL, line_w=0.75,
             shape_type=MSO_SHAPE.ROUNDED_RECTANGLE, adj=0.04)
    add_rect(slide, card_l + 0.60, card_t + 3.45, 0.10, 1.40, ACCENT_SAFFRON)
    add_text_box(slide, card_l + 0.90, card_t + 3.60, card_w - 1.80, 0.40, stat.get("headline", "5,000 Years of Living Thought"),
                 size=15, bold=True, color=ACCENT_METAL_LIGHT, font=TITLE_FONT)
...
def run_audit(pptx_path, expected_slides=11):
    ...
    # 3. Shape bounding box and speaker notes check
...
def main():
    print("================================================================")
    print("PPTmaker: Building Indian Culture Presentation (.pptx)")
    print("================================================================")
    ...
    # Core properties
    prs.core_properties.title = master_spec.get("working_title", "The Living Civilization: Indian Culture")
    prs.core_properties.author = "PPTmaker Autonomous Presentation Studio"
    prs.core_properties.subject = "5,000 Years of Indian Culture, Philosophy, and Heritage"
    prs.core_properties.keywords = "Indian Culture, Heritage, Philosophy, Yoga, Architecture, Bharatanatyam, History"
    ...
    out_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Indian_Culture_Presentation.pptx")
    prs.save(out_file)
    ...
    if errors:
        ...
        print("  ✓ High-resolution cultural visual assets integrated")
